...
public class LDAPSession extends BaseSession implements EntrySource {
    ...
    public DirContext getContext() {
        if (dirContext == null) {
            dirContext = LdapRetryHandler.wrap(getDirectory().createContext(), getDirectory().getServer().getRetries());
        }
        return dirContext;
    }
    ...
    public DocumentModelList getEntries() throws DirectoryException {
        ...
        try {
            ...
            NamingEnumeration<SearchResult> results = getContext().search(searchBaseDn, getDirectory().getBaseFilter(), scts);
    ...
    public DocumentModelList query(Map<String, Serializable> filter, Set<String> fulltext, boolean fetchReferences,
        ...
        try {
            ...
            try {
                NamingEnumeration<SearchResult> results = getContext().search(searchBaseDn, filterExpr, filterArgs, scts);
