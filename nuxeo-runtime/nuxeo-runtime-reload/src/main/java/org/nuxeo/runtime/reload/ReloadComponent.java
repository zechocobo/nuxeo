...
public class ReloadComponent extends DefaultComponent implements ReloadService {
    ...
    protected void refreshComponents() {
        String reloadStrategy = Framework.getProperty("org.nuxeo.runtime.reload_strategy", "standby");
    ...
    @Override
    public void deployBundles(File[] files, boolean reloadResources) throws BundleException {
        List<String> missingNames = Stream.of(files)
        ...
        if (log.isInfoEnabled()) {
            // TODO REVIEW - not logging each paths to avoid looping files once again, should we log still ?
        ...
        // Reload resources
        if (reloadResources) {
            List<URL> urls = Stream.of(files).map(this::toURL).collect(Collectors.toList());
    ...
    public void undeployBundle(File file, boolean reloadResources) throws BundleException {
        ...
        undeployBundle(name, reloadResources);
    }

    @Override
    public void undeployBundles(String[] bundleNames, boolean reloadResources) throws BundleException {
        if (log.isInfoEnabled()) {
            // TODO REVIEW - not logging each paths to avoid looping bundleNames once again, should we log still ?
        ...
        Transaction tx = TransactionHelper.suspendTransaction();
        // use a stream builder cause it is optimized for one element only
    ...
    protected URL toURL(File file) {
        try {
            ...
            // TODO REVIEW - should we replace it by RuntimeServiceException?
    ...
    /**
     * @deprecated since 9.3 should not bet needed anymore
     */
    @Deprecated
